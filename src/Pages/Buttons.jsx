...
const Buttons = () => {
    ...
    return (
        <Grid textAlign={'center'} mt={'1vh'} templateColumns={['repeat(2, 1fr)', 'repeat(3, 1fr)', 'repeat(4, 1fr)', 'repeat(4, 1fr)']} gap={3}>
            {
                buttonText.map((ele, i) => {
                    return <GridItem key={i}>
                        <Button onClick={()=>handleQuery(ele.q)} _hover={{color:"none"}} fontSize={15} color={'white'} boxShadow='2xl' bg={'#4285f4'} w={'100%'} borderRadius={'none'} >{ele.title}</Button>
                    </GridItem>
                })
            }
        </Grid>
